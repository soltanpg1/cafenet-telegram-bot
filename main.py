...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # مشتری: دریافت درخواست مدارک بیشتر
    if data.startswith("send_more_docs:"):
        code = data.split(":", 1)[1]
        order = db.get_order(code, user.id)
        if not order:
            await query.answer("❌ سفارش پیدا نشد.", show_alert=True)
            return
        db.set_awaiting_documents(code, False)
        context.user_data["stage"] = "file"
        context.user_data["last_code"] = code
        await query.edit_message_text(
            f"📎 ارسال مدارک بیشتر برای سفارش #{code}\n\n"
            "لطفاً مدارک موردنیاز را همین‌جا ارسال کنید.",
    ...
    try:
        ...
        if data == "admin_change_status":
            ...
            rows = [[InlineKeyboardButton(
                f"#{o['code']} | {o['service_name'][:25]}",
                callback_data=f"change_status:{o['code']}"
            )] for o in orders[:30]]
        ...
        if data.startswith("change_status:"):
            ...
            order = db.get_order(code)
            if not order:
                await query.edit_message_text("❌ سفارش پیدا نشد.", reply_markup=kb.admin_panel())
                return
            await query.edit_message_text(
                f"🔄 تغییر وضعیت سفارش #{code}\n\n"
                f"وضعیت فعلی: {status_name(order['status'])}\n\n"
                "وضعیت جدید را انتخاب کنید:",
                reply_markup=kb.status_change_menu(code)
            )
            return

        if data.startswith("set_status:"):
            _, code, status = data.split(":", 2)
            order = db.get_order(code)
            if not order:
                await query.edit_message_text("❌ سفارش پیدا نشد.", reply_markup=kb.admin_panel())
                return
            if status == "completed" and not db.can_admin_complete(user.id):
                await query.answer(
                    "⛔ سهمیه تکمیل سفارش امروز شما پر شده است.",
                    show_alert=True
                )
                return
            ok = db.set_status(code, status, user.id, admin_name(user),
                               f"تغییر دستی وضعیت به {status_name(status)} توسط ادمین")
            if ok:
                order = db.get_order(code)
                try:
                    await context.bot.send_message(
                        chat_id=order["user_id"],
                        text=f"📌 وضعیت سفارش #{code} تغییر کرد.\n\n"
                             f"وضعیت جدید: {status_name(status)}"
                    )
                except Exception as e:
                    logger.error("Status notification error: %s", e)
                await query.edit_message_text(
                    order_text(order),
                    reply_markup=kb.admin_order_buttons(code, status)
                )
            return
        ...
        # درخواست مدارک بیشتر
        # -----------------------------
        if data == "admin_request_docs":
            orders = db.get_all_orders()
            rows = [[InlineKeyboardButton(
                f"#{o['code']} | {o['service_name'][:25]}",
                callback_data=f"request_docs:{o['code']}"
            )] for o in orders[:30]]
            rows.append([InlineKeyboardButton("🔙 پنل مدیریت", callback_data="admin_panel")])
            await query.edit_message_text(
                "📎 درخواست مدارک بیشتر\n\nسفارش موردنظر را انتخاب کنید:",
        ...
        if data.startswith("request_docs:"):
            ...
            if not order:
                await query.edit_message_text("❌ سفارش پیدا نشد.", reply_markup=kb.admin_panel())
                return
            db.set_awaiting_documents(code, True)
            db.set_status(
                code, "under_review", user.id, admin_name(user),
                "درخواست مدارک بیشتر از مشتری"
            )
            order = db.get_order(code)
            try:
                await context.bot.send_message(
                    chat_id=order["user_id"],
                    text=f"📎 برای سفارش #{code} به مدارک بیشتری نیاز داریم.\n\n"
                         "لطفاً مدارک موردنیاز را در همین چت ارسال کنید.",
                    reply_markup=kb.customer_document_button(code)
                )
            except Exception as e:
                logger.error("Document request error: %s", e)
            await query.edit_message_text(
                f"✅ درخواست مدارک بیشتر برای سفارش #{code} برای مشتری ارسال شد.",
                reply_markup=kb.admin_order_buttons(code, order["status"])
            )
            return
        ...
        if data.startswith("reject_order:"):
            ...
            if not order:
                await query.edit_message_text("❌ سفارش پیدا نشد.", reply_markup=kb.admin_panel())
                return
            db.set_status(code, "rejected", user.id, admin_name(user), "سفارش توسط ادمین رد شد")
            try:
                await context.bot.send_message(
                    chat_id=order["user_id"],
                    text=f"❌ سفارش #{code} رد شد.\n\n"
                         "متأسفانه امکان تأیید یا انجام این سفارش وجود ندارد."
                )
            except Exception as e:
                logger.error("Reject notification error: %s", e)
            await query.edit_message_text(
                f"❌ سفارش #{code} رد شد.",
                reply_markup=kb.admin_panel()
            )
        ...
        if data.startswith("complete:"):
            ...
            ok, reason = db.complete_order(code, user.id, admin_name(user))
            if not ok:
                msg = {
                    "quota_reached": "⛔ سهمیه تکمیل سفارش امروز شما پر شده است.",
                    "already_completed": "⚠️ این سفارش قبلاً تکمیل شده است.",
                    "not_found": "❌ سفارش پیدا نشد."
                }.get(reason, "❌ تکمیل سفارش انجام نشد.")
            ...
            try:
                await context.bot.send_message(
                    chat_id=order["user_id"],
                    text=f"✅ سفارش #{code} با موفقیت تکمیل شد.\n\n"
                         "از اعتماد شما به کافینت آنلاین سپاسگزاریم. 🌹",
                    reply_markup=kb.main_menu()
                )
            except Exception as e:
                logger.error("Completion notification error: %s", e)
        ...
        if data == "admin_reports":
            ...
            limit = db.get_daily_order_limit()
            today_count = db.get_daily_order_count()
            status_lines = "\n".join(
                f"{status_name(s)}: {counts.get(s, 0)}"
                for s in ("new","in_progress","waiting_payment","under_review","completed","rejected")
            )
            await query.edit_message_text(
                "📊 وضعیت سفارش‌ها و گزارش\n\n"
                "📌 وضعیت فعلی سفارش‌ها:\n"
                f"{status_lines}\n\n"
                f"📅 هفته گذشته: {stats['week']} سفارش\n"
                f"🗓 ماه گذشته: {stats['month']} سفارش\n"
                f"📆 سال گذشته: {stats['year']} سفارش\n\n"
                f"📌 سفارش‌های ثبت‌شده امروز: {today_count}\n"
                f"🎯 سقف امروز: {'بدون محدودیت' if limit <= 0 else limit}",
                reply_markup=kb.admin_settings()
            )
            return

        if data == "daily_order_limit":
            limit = db.get_daily_order_limit()
            await query.edit_message_text(
                f"🎯 محدودیت ثبت سفارش روزانه\n\n"
                f"سقف فعلی: {'بدون محدودیت' if limit <= 0 else limit}\n\n"
                "عدد موردنظر را ارسال کنید.\n"
                "برای برداشتن محدودیت، ۰ ارسال کنید.",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔙 تنظیمات", callback_data="admin_settings")]
                ])
            )
            context.user_data["admin_action"] = "daily_order_limit"
            return
        ...
        if data == "change_card":
            if not is_super_admin(user.id):
                ...
                return
            await query.edit_message_text(
                f"💳 شماره کارت فعلی:\n{db.get_setting('card_number', CARD_NUMBER)}\n\n"
                "شماره کارت جدید را ارسال کنید:"
            )
            context.user_data["admin_action"] = "change_card"
            return

        if data == "change_card_name":
            if not is_super_admin(user.id):
                await query.answer("⛔ فقط ادمین اصلی.", show_alert=True)
                return
            await query.edit_message_text(
                f"🏷 نام شماره کارت فعلی:\n{db.get_setting('card_name', 'نام دارنده کارت')}\n\n"
                "نام جدید را ارسال کنید:"
            )
            context.user_data["admin_action"] = "change_card_name"
            return
        ...
        if data == "admin_quotas":
            ...
            rows = []
            for a in admins:
                if a["user_id"] in admin_ids_from_config():
                    continue
                q = db.get_admin_daily_limit(a["user_id"])
                rows.append([InlineKeyboardButton(
                    f"👨‍💼 {a['full_name'] or a['user_id']} | سهم: {'∞' if q <= 0 else q}",
                    callback_data=f"set_quota:{a['user_id']}"
                )])
            rows.append([InlineKeyboardButton("🔙 مدیریت ادمین‌ها", callback_data="admin_manage")])
            await query.edit_message_text(
        ...
        if data.startswith("set_quota:"):
            ...
            target = db.get_admin(target_id)
            if not target:
                await query.edit_message_text("❌ ادمین پیدا نشد.", reply_markup=kb.admin_management())
                return
            context.user_data["admin_action"] = "set_quota"
            context.user_data["quota_admin_id"] = target_id
            await query.edit_message_text(
                f"🎯 سهم روزانه {target['full_name'] or target_id}\n\n"
                "تعداد سفارش قابل تکمیل در روز را ارسال کنید.\n"
                "۰ یعنی بدون محدودیت."
            )
            return
...
async def text_message(
    ...
    if is_admin(user.id):
        ...
        if text == "👨‍💻 پنل مدیریت":
            context.user_data["admin_mode"] = True
            await update.message.reply_text("👨‍💻 پنل مدیریت", reply_markup=kb.admin_panel())
            return

        if text == "📞 پیام‌های پشتیبانی":
            await update.message.reply_text(
                "💬 پیام‌های پشتیبانی\n\nپیام‌های مشتریان از طریق اعلان‌ها برای ادمین ارسال می‌شوند.",
                reply_markup=kb.admin_panel()
            )
            return
        ...
        if action == "daily_order_limit":
            value = normalize_digits(text).strip()
            if not value.isdigit() or int(value) < 0:
                await update.message.reply_text("❌ فقط یک عدد صفر یا بزرگ‌تر ارسال کنید.")
                return
            db.set_setting("daily_order_limit", int(value))
            context.user_data.pop("admin_action", None)
            await update.message.reply_text(
                f"✅ سقف ثبت سفارش روزانه روی {'بدون محدودیت' if int(value)==0 else int(value)} تنظیم شد.",
                reply_markup=kb.admin_reply_keyboard()
            )
            return

        if action == "change_card":
            value = normalize_digits(text).replace(" ", "").replace("-", "")
            if not value.isdigit() or len(value) != 16:
                await update.message.reply_text("❌ شماره کارت باید ۱۶ رقم باشد.")
                return
            db.set_setting("card_number", value)
            context.user_data.pop("admin_action", None)
            await update.message.reply_text("✅ شماره کارت تغییر کرد.", reply_markup=kb.admin_reply_keyboard())
            return

        if action == "change_card_name":
            if len(text) < 2:
                await update.message.reply_text("❌ نام واردشده معتبر نیست.")
                return
            db.set_setting("card_name", text)
            context.user_data.pop("admin_action", None)
            await update.message.reply_text("✅ نام شماره کارت تغییر کرد.", reply_markup=kb.admin_reply_keyboard())
            return

        if action == "set_quota":
            value = normalize_digits(text).strip()
            target_id = context.user_data.get("quota_admin_id")
            if not value.isdigit() or int(value) < 0 or not target_id:
                await update.message.reply_text("❌ یک عدد صفر یا بزرگ‌تر ارسال کنید.")
                return
            db.set_setting(f"admin_daily_limit:{target_id}", int(value))
            context.user_data.pop("admin_action", None)
            context.user_data.pop("quota_admin_id", None)
            await update.message.reply_text(
                f"✅ سهم روزانه ادمین روی {'بدون محدودیت' if int(value)==0 else int(value)} تنظیم شد.",
                reply_markup=kb.admin_reply_keyboard()
            )
            return
...
async def file_message(
    ...
    if not code:
        ...
        if not code:
            await update.message.reply_text(
                "⛔ ظرفیت ثبت سفارش امروز تکمیل شده است.\n\n"
                "لطفاً فردا دوباره مراجعه کنید.",
                reply_markup=kb.main_menu()
            )
            return
...
async def pay_callback(
    ...
    await query.edit_message_text(
        ...
        f"{order['amount']:,} تومان\n\n"
        f"💳 شماره کارت:\n"
        f"{db.get_setting('card_number', CARD_NUMBER)}\n\n"
...
async def send_completed_report(context, order):
    chat_id = os.environ.get("COMPLETED_ORDERS_CHAT_ID") or os.environ.get("REPORT_CHAT_ID")
    if not chat_id:
        return
    try:
        await context.bot.send_message(
            ...
            text=(
                "✅ گزارش تکمیل سفارش\n\n"
                f"📦 کد سفارش: #{order['code']}\n"
                f"👤 نام مشتری: {order['full_name'] or '—'}\n"
                f"📱 موبایل: {order['phone'] or '—'}\n"
                f"🆔 Telegram ID: {order['user_id']}\n"
                f"🛠 خدمت: {order['service_name']}\n"
                f"💰 مبلغ: {order['amount']:,} تومان\n"
                f"📎 تعداد مدارک: {db.get_file_count(order['code'])}\n"
                f"📌 وضعیت: {status_name(order['status'])}\n"
                f"👨‍💼 انجام‌دهنده: {order['admin_name'] or '—'}\n"
                f"📝 توضیحات: {order['description'] or 'ندارد'}\n"
                f"🕐 ثبت سفارش: {order['created_at']}\n"
                f"🕐 تکمیل: {order['completed_at'] or '—'}"
            )
        )
        # مدارک سفارش نیز برای آرشیو به گروه/کانال گزارش ارسال می‌شوند.
        for f in db.get_order_files(order["code"]):
            try:
                if f["file_type"] == "photo":
                    await context.bot.send_photo(
                        chat_id=chat_id,
                        photo=f["telegram_file_id"],
                        caption=f"📎 مدرک سفارش #{order['code']}"
                    )
                else:
                    await context.bot.send_document(
                        chat_id=chat_id,
                ...
                logger.error("Completed-order file report error: %s", file_error)
    except Exception as e:
        logger.error("Completed-order report error: %s", e)
